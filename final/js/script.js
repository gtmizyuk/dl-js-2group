...
if (!localStorage["moviesProSaveStorage"] && !localStorage["allStarsСlubMembersSaveStorage"]) {
    initLocalStorage();
} 

let dataMovies = JSON.parse(localStorage["moviesProSaveStorage"]);
let dataMembersClub = JSON.parse(localStorage["allStarsСlubMembersSaveStorage"]);    
...
function getIdCardActor() {
    ...
    if (dataLength == 2 || dataLength == 1) {
        ...
        let numberClubMembers = allStarsСlubMembers.getNumberClubMembers(dataMembersClub);
        let numberClubMembersNew = numberClubMembers;
...
function btnAdminLogOut() {
    let sessionMoviewsPro = sessionStorage.getItem('moviesProSaveSession');
...
function btnAdminSignIn() {
    ...
    if (loginCheck) {
        ...
        //console.log(dataMovies, "Session start...");
        //console.log(dataMembersClub, "Session start...");
...
function getArrayReviewByLastDate(dataMovies) {
    let arrayLastReviewStat = moviesPro.getLastReviewStatistics(dataMovies);
    return arrayLastReviewStat
}

function staticticsLastReview(dataMovies) {
    let arrayLastReview = getArrayReviewByLastDate(dataMovies);
...
function addNewMovie() {
    ...
    if (arrayAddNewMovie.length == 3) {
        ...
        let dmSession = JSON.parse(sessionStorage.getItem("moviesProSaveSession"));
...
function initiative() {
    ...
    if (sm) {
        ...
        if (isCorrectStorage()) {
            ...
            //console.log(dataMoviesSession, "MoviesPro SESSION");
            //console.log(dataMembersClubSession, "Сlub Members SESSION");
